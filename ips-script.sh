#!/bin/bash
sed -i "s#.*ansible_host.*#ansible_host: $(terraform -chdir=./terraform  output -raw public-ip-ec2 )#" ./ansible/group_vars/proxy.yaml
sed -i "s#HostName.*#HostName $(terraform -chdir=./terraform  output -raw public-ip-ec2)#" /var/jenkins_home/.ssh/config
